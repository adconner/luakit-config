...
add_binds("normal", {
    ...
    buf("jj$",                      function (w) w:set_mode()          end),
    ...
    key({},          "d",           function (w) w:scroll{ y = "+0.5p" } end),
    key({},          "u",           function (w) w:scroll{ y = "-0.5p" } end),
    ...
    buf("^a$",                      function (w, c) w:enter_cmd(":bookmark " .. (w.view.uri or "") .. " ") end),
    ...
    key({"Control"}, "h",           function (w)       w:prev_tab() end),
    key({"Control"}, "l",           function (w)       w:next_tab() end),
    ...
    -- not working, using quvi.lua instead
    --buf("^qq$",                     function (w) luakit.spawn('urxvt -e sh -c \'quvi -f best -exec "mplayer %u" "' .. string.gsub(w.view.uri or "", " ", "%%20") .. '" | less\'') end),
    --buf("^qd$",                     function (w) luakit.spawn('quvi -exec "mplayer %u" ' .. string.gsub(w.view.uri or "", " ", "%%20")) end),
...
add_cmds({
 ...
 -- cmd("co[mmand]",            function (w, arg, opts) .. end, opts),
    ...
    cmd({"source",  "vs" }, function (w, a, o) w:toggle_source() end),
